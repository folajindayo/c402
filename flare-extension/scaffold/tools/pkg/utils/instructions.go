...
func SendAssessCredit(s *support.Support, instructionSenderAddress common.Address, message []byte) (common.Hash, common.Hash, error) {
	sender, err := helloworld.NewHelloWorldInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	tx, err := sender.SendAssessCredit(opts, message)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := helloworld.HelloWorldInstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("sendAssessCredit", message)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
